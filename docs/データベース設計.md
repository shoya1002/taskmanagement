...
| Card（カード） | id, column_id, text, order, priority | 1件のタスク。必ずいずれか1つの Column に属する。`order` はリスト内での表示順序。`priority` は `"high"` / `"medium"` / `"low"` のいずれか（初期値 `"medium"`） |
...
| priority | string | NOT NULL | 優先度。`"high"` / `"medium"` / `"low"` のいずれか（初期値 `"medium"`） |
...
```json
[
  { "id": "card-1", "column_id": "todo",        "text": "タスクA", "order": 0, "priority": "high"   },
  { "id": "card-2", "column_id": "todo",        "text": "タスクB", "order": 1, "priority": "medium" },
  { "id": "card-3", "column_id": "in-progress", "text": "タスクC", "order": 0, "priority": "low"    },
  { "id": "card-4", "column_id": "done",        "text": "タスクD", "order": 0, "priority": "medium" }
]
```
...
| カードを追加 | `db.cards.add({ id, column_id, text, order, priority })` |
| カードを削除 | `db.cards.delete(cardId)` |
| カードを別列へ移動（トランザクション） | `db.transaction('rw', db.cards, () => db.cards.update(cardId, { column_id, order }))` |
| カードの優先度を更新 | `db.cards.update(cardId, { priority })` |

`priority` は列ごとの表示順序（優先度順・期限順）の切り替えにのみ使用し、その並び替えは DB から取得済みのカード一覧をメモリ上でソートして行う（`order` の値自体は変更しない）。データ量が少量（1ボード・少数カード）のため、`priority` にインデックスは付与しない。
